...
void download_data(string ticker, string start_date, string end_date){
    string url = make_url(ticker, start_date, end_date);
    string cmd = "wget \"" + url + "\" -O data.csv";
    system( cmd.c_str() );
}

string make_url(string ticker, string start_date, string end_date){
...
int calc_stats(int stat_choice, int data_choice){
    /* Return Codes
     * 0 - Successful run
